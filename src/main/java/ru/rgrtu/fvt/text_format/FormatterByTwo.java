...
public class FormatterByTwo extends Formatter {
    ...
    public String format(String text) {
        ...
        rows.forEach(line -> {
            ...
            for (Integer i = rightLine; i < array.length; ) {
                ...
                if (letterData.getSimbol()) {//если индекс для переноса символ переносить без дефиса, иначе буква тогда с дефисом
                    resultText.append("\r\n");
                } else {
    ...
    private LetterData getLetterIndex(char[] array, int startIndex) {
        if (Character.isLetter(array[startIndex])) {//если текущий индекс явл буквой, то ищется ближайшее совпадение букв для перевода, если нет, то тек индекс считаеться успешным для переноса
